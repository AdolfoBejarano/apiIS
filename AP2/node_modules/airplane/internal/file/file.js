...
exports.upload = exports.AirplaneFile = void 0;
const mime_1 = __importDefault(require("mime"));
const file_object_1 = require("./file_object");
...
const upload = async (payload, fileName) => {
    if (isBlob(payload)) {
        if (!fileName) {
            const extension = mime_1.default.getExtension(payload.type);
            if (!extension) {
    ...
    const upload = await (0, runtime_1.getRuntime)().createUpload(fileName, payload.size);
    await (0, runtime_1.getRuntime)().uploadFileContents(upload.writeOnlyURL, payload);
    const file = {
        __airplaneType: "upload",
        id: upload.upload.id,
