...
class Client {
    ...
    async executeTask(slug, paramValues, resources, opts = {}) {
        return this.executeTaskWithCacheInfo(slug, paramValues, resources, opts).then((resp) => resp.runID);
    }
    async executeTaskWithCacheInfo(slug, paramValues, resources, opts = {}) {
    ...
    async executeTaskAirplane(slug, paramValues, resources, opts = {}) {
        ...
        return this.withTracingPOST(path, async (headers) => {
            try {
                const { runID, isCached } = await this.fetcher.post(path, {
                    slug: slug,
                ...
                return { runID, isCached };
            }
            catch (e) {
                if (e instanceof lib_1.HTTPError) {
    ...
    async executeTaskSelfHostedInputs(slug, paramValues, resources, opts = {}) {
        ...
        return this.withTracingPOST(airplanePath, async (headers) => {
            var _a;
            try {
                const { runID, isCached } = await this.fetcher.post(airplanePath, {
                    slug: slug,
                    ...
                    inputsZoneParamValuesHash: agentResp.paramValuesHash,
                    allowCachedMaxAge: this.allowCachedMaxAge,
                }, {
                    abortSignal: opts.abortSignal,
                    idempotencyKey: opts.idempotencyKey,
