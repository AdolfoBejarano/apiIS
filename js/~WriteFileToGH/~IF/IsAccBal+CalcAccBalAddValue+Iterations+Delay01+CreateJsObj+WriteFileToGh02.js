...
isaccsonBal[i][3] = isoutputData.balance;
isaccsonBal[i][4] = (values [i] [1])-isoutputData.balance;
//console.log(i,(values [i] [0]),':',(values [i] [1]),'-',isaccsonBal[i][3],'=',isaccsonBal[i][4]);
//console.log(isaccsonBal);

isoutputDataA = isaccsonBal;
isoutputFunctA();
}
)
...
async function isoutputFunctA()
{
let content = isoutputDataA;
...
let dataB = JSON.stringify({message: message,content: btoa(content),});
