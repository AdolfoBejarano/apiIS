...
let HHMM = HH+''+MM;
let HHMMSS = HH+''+MM+''+SS;
const FileName = `${(new Date().toJSON().slice(0,10))}.${HHMMSS}_IsAccBalAllOnAccs.csv`;
...
//fs.writeFile(`${filePath}/metadata.json`, data, (error) =>{if (error) {console.error(error);throw error;}console.log("metadata.json written correctly");});

let shared = require('./WriteFileToGD_ScriptB.js');
console.log('at@A:',shared.gdgenatbyrt());
//let z= (shared.gdgenatbyrt());
//console.log('z:',z);
//console.log('y:',y);
...
isacc = values [i][0];
let shared = require('./WriteFileToGD_ScriptC.js');
console.log('FinalOutcomeB:',i,arrayL-2,shared.writefiletoPC());
*/
